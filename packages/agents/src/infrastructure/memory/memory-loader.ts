...
export class MemoryLoader {
  private static dataSource: MemoryDataSource | null = null;
  ...
  static async load(
    ...
    try {
      ...
      const systemPromptInjection = this.buildSystemPromptInjection(
        workspaceMemory, projectMemory, brandProfile,
      );
      ...
      log.error('Failed to load memory context, using empty', {}, error as Error);
      return this.emptyContext();
    }
  }
  ...
  private static buildSystemPromptInjection(
    workspace: AIMemory | null,
    project: AIMemory | null,
  ...
  private static emptyContext(): MergedMemoryContext {
    return {
