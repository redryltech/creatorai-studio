...
import { AgentId, ContentType, Platform, ScriptStyle, AgentError, estimateSpeakingDuration } from '@creatorai/shared';
...
export interface ScriptAgentInput {
  ...
  duration?: number;           // seconds; defaults from platform spec
  style: ScriptStyle;
  tone: string;
  language: string;
  keyPoints?: string[];
...
export class ScriptAgent extends BaseAgent<ScriptAgentInput, ScriptAgentOutput> {
  ...
  protected async doValidate(input: ScriptAgentInput): Promise<ValidationResult> {
  ...
  protected async doExecute(
  ...
  ): Promise<{ data: ScriptAgentOutput; metrics?: { tokensUsed?: number; costUsd?: number; provider?: string } }> {
    ...
    try {
      ...
      agentLog.error('Failed to parse LLM JSON response', {}, parseError as Error);
      throw new AgentError(
        this.id,
        `Script generation returned invalid JSON. Model: ${response.model}. Finish reason: ${response.finishReason}`,
        true, // Retryable — LLM may produce valid JSON on next attempt
      );
    }
  ...
  protected async doEstimateCost(input: ScriptAgentInput): Promise<CostEstimate> {
  ...
  private calculateSceneCount(duration: number, isShortForm: boolean): number {
    if (isShortForm) {
   ...
   */
  private normalizeScript(raw: any, input: ScriptAgentInput, targetDuration: number): Script {
    const scenes = (raw.scenes ?? []).map((s: any, i: number) => ({
      id: s.id ?? `scene-${i + 1}`,
      order: s.order ?? i + 1,
      type: s.type ?? 'body',
    ...
    const fullText = raw.fullText ?? raw.full_text ?? scenes.map((s: any) => s.narration).join(' ');
   ...
   */
  private validateScript(script: Script, targetDuration: number, expectedScenes: number): void {
