...
export class PromptManager {
     ...
     */
    render(templateId, variables, overrides = {}) {
        const template = this.templates.get(templateId);
        if (!template) {
            throw new Error(`Prompt template "${templateId}" not found. Available: ${this.listIds().join(', ')}`);
        }
        // Validate all required variables are provided
        const missingVars = template.variables.filter((v) => !(v in variables));
        if (missingVars.length > 0) {
            throw new Error(`Missing variables for template "${templateId}": ${missingVars.join(', ')}`);
        }
