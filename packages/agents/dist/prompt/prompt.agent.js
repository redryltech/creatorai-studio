...
export class PromptAgent extends BaseAgent {
    ...
    version = '1.0.0';
    description = 'Generates optimized AI image/video prompts from script scenes';
    getMetadata() {
        return {
            id: this.id,
            name: this.name,
    ...
    async doExecute(input, context) {
        ...
        const scenesText = input.scenes.map((s) => `Scene ${s.order} (${s.type}): Narration: "${s.narration}" | Visual: "${s.visualDescription}" | Duration: ${s.duration}s`).join('\n\n');
        ...
        const scenePrompts = (parsed.scenePrompts ?? parsed.scene_prompts ?? []).map((p, i) => ({
