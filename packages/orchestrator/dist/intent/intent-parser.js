...
export class IntentParser {
    ...
    async parse(message, userId) {
        ...
        const llm = await providerRegistry.getPrimary('llm');
        if (!llm) {
        ...
        try {
            ...
            log.error('Intent parsing failed, falling back to general_chat', {}, error);
            return this.createGeneralChatIntent(trimmed);
        }
    }
    /**
    ...
    normalize(raw, originalMessage) {
        ...
        const missingRequired = raw.missingRequired ?? [];
        if (this.isCreationAction(action) && !entities.topic) {
            if (!missingRequired.includes('topic'))
    ...
    resolveAction(raw) {
        if (!raw)
            return IntentAction.GENERAL_CHAT;
    ...
    generateClarificationQuestion(action, missing) {
    ...
    createGeneralChatIntent(message) {
        return {
