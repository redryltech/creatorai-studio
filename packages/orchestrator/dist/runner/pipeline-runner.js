...
// ============================================================
import { PipelineStatus, StepStatus, PipelineError, sleep, } from '@creatorai/shared';
...
export class PipelineRunner {
    ...
    eventBus;
    pipelineRepo;
    cancelledPipelines = new Set();
    constructor(pipelineRepo, config = {}) {
    ...
    async run(pipeline) {
        ...
        const context = createAgentContext({
            ...
            userId,
            onProgress: (update) => {
                this.eventBus.emitStepProgress(pipelineId, projectId, userId, pipeline.currentStep ?? '', update.progress, update.message);
            },
            onLog: (entry) => {
        ...
        try {
            ...
            while (true) {
                ...
                const readySteps = steps.filter((step) => step.status === StepStatus.PENDING &&
                    step.dependsOn.every((depId) => completedSteps.has(depId) || failedSteps.has(depId)));
                // Check if we're done
                if (readySteps.length === 0) {
                    const allDone = steps.every((s) => s.status === StepStatus.COMPLETED ||
                    ...
                    if (pendingSteps.length > 0) {
                        throw new PipelineError(pipelineId, `Pipeline deadlock: ${pendingSteps.length} steps pending but no steps are ready`);
                    }
                    break;
                }
                // Execute ready steps in parallel (limited concurrency)
                const batch = readySteps.slice(0, this.config.maxConcurrency);
                const results = await Promise.allSettled(batch.map((step) => this.executeStep(step, context, pipelineId, projectId, userId)));
                // Process results
                for (let i = 0; i < results.length; i++) {
                    ...
                    if (result.status === 'fulfilled' && result.value.success) {
                        ...
                        const isBlocking = steps.some((s) => s.dependsOn.includes(step.id) && !this.isStepOptional(s, steps));
                ...
                const completedCount = steps.filter((s) => s.status === StepStatus.COMPLETED || s.status === StepStatus.SKIPPED).length;
     ...
     */
    async executeStep(step, context, pipelineId, projectId, userId) {
        ...
        for (let attempt = 0; attempt <= step.maxRetries; attempt++) {
            try {
                ...
                if (result.success) {
                    // Step succeeded
                    await this.pipelineRepo.markStepCompleted(pipelineId, step.id, result.data);
                    this.eventBus.emitStepCompleted(pipelineId, projectId, userId, step.id, result.data);
                    return result;
                }
                ...
                if (!result.error?.retryable || attempt >= step.maxRetries) {
                    ...
                    this.eventBus.emitStepFailed(pipelineId, projectId, userId, step.id, result.error?.message ?? 'Unknown error', false);
                    return result;
                }
                // Retry
                lastError = new Error(result.error.message);
                this.eventBus.emitStepFailed(pipelineId, projectId, userId, step.id, result.error.message, true);
                const delay = Math.min(this.config.retryBaseDelayMs * Math.pow(2, attempt), this.config.retryMaxDelayMs);
                await sleep(delay);
            }
            catch (error) {
                ...
                if (attempt >= step.maxRetries) {
                    ...
                    this.eventBus.emitStepFailed(pipelineId, projectId, userId, step.id, lastError.message, false);
                    throw lastError;
                }
                const delay = Math.min(this.config.retryBaseDelayMs * Math.pow(2, attempt), this.config.retryMaxDelayMs);
                await sleep(delay);
     ...
     */
    skipDependentSteps(failedStepId, steps) {
