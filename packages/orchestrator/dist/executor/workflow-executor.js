...
// ============================================================
import { AgentRegistry, createAgentContext, Logger } from '@creatorai/agents';
...
export class WorkflowExecutor {
     ...
     */
    async execute(plan, intent, userId, projectId) {
        ...
        const run = {
            ...
            progress: 0,
            currentNodeId: null,
            nodeStates: new Map(plan.nodes.map((n) => [n.id, this.createInitialNodeState(n.id)])),
            startedAt: new Date(),
            completedAt: null,
            error: null,
            totalCostUsd: 0,
    ...
    // DAG Execution Loop
    // ================================================================
    async executeDAG(plan, intent, run, artifacts) {
        ...
        while (true) {
            ...
            if (this.pausedRuns.has(run.id)) {
                ...
                if (this.cancelledRuns.has(run.id))
                    throw new Error('Workflow cancelled while paused');
                run.status = WorkflowStatus.RUNNING;
                this.events.emit(WorkflowEventType.WORKFLOW_RESUMED, run, {});
            }
            // Find all runnable nodes
            const runnableIds = plan.executionOrder.filter((id) => isRunnable(id) && !activeNodes.has(id));
            ...
            if (runnableIds.length === 0 && activeNodes.size === 0) {
                ...
                const allDone = [...run.nodeStates.values()].every((s) => s.status === NodeStatus.COMPLETED || s.status === NodeStatus.SKIPPED);
                if (allDone)
                    return;
            ...
            if (batch.length > 0) {
                ...
                const completedCount = [...run.nodeStates.values()].filter((s) => s.status === NodeStatus.COMPLETED || s.status === NodeStatus.SKIPPED).length;
    ...
    // ================================================================
    async executeNode(node, intent, run, artifacts) {
     ...
     */
    resolveInputMappings(mappings, intent, artifacts) {
        const resolved = {};
        for (const [field, mapping] of Object.entries(mappings)) {
            let value;
            switch (mapping.source) {
                case 'intent':
                    value = intent.entities[mapping.key];
                    break;
                case 'artifact':
    ...
    handleNodeFailure(node, state, run, errorMessage) {
    ...
    skipDependentNodes(failedNodeId, run) {
    ...
    mapAgentToArtifactType(agentId) {
        const mapping = {
