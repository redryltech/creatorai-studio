...
export class ConversationOrchestrator {
  ...
  private readonly activeWorkflows: Map<string, Promise<OrchestratorWorkflowResult>> = new Map();
   ...
   */
  async waitForWorkflow(runId: string): Promise<OrchestratorWorkflowResult | null> {
   ...
   */
  private async handleGeneralChat(message: string, userId: string): Promise<string> {
    try {
      const registry = ProviderRegistry.getInstance();
      const llm = await registry.getPrimary<ILLMProvider>('llm');
      if (!llm) return 'I\'m here to help you create content! Try saying something like "Create 5 YouTube Shorts about AI technology."';
      ...
      log.error('General chat failed', {}, error as Error);
   ...
   */
  private buildPlanSummary(intent: ParsedIntent, plan: WorkflowPlan): string {
