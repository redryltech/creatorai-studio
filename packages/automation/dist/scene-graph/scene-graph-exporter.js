    ...
    static export(pkg) {
        return {
            ...
            debugGraph: pkg.scenes.map((g) => {
                ...
                const nodes = Object.values(g.nodes);
                if (nodes.length < 3)
                    issues.push('Very few nodes — scene may be empty');
                if (g.lightNodes.length === 0)
                    issues.push('No lights');
                if (!g.cameraNode.targetNodeId)
                    issues.push('Camera has no target');
                const noEntity = nodes.filter((n) => n.type === 'vehicle' || n.type === 'character').length;
                if (noEntity === 0)
                    issues.push('No characters or vehicles');
                return { sceneId: g.sceneId, issues };
            }),
        };
    }
}
//# sourceMappingURL=scene-graph-exporter.js.map