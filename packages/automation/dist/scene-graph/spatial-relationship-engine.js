    ...
    static analyze(graph) {
        ...
        for (const a of importantNodes) {
            ...
            for (const b of importantNodes) {
                ...
                // Interaction type
                if (dist < 2)
                    interactionMatrix[a.id][b.id] = 'contact';
                else if (dist < 5)
                    interactionMatrix[a.id][b.id] = 'near';
                else if (dist < 15)
                    interactionMatrix[a.id][b.id] = 'visible';
                else
                    interactionMatrix[a.id][b.id] = 'distant';
            }
        }
...
//# sourceMappingURL=spatial-relationship-engine.js.map