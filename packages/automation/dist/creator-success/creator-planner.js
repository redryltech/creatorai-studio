...
export class CreatorPlanner {
    ...
    static plan(input) {
        ...
        const publishing = PublishingEngine.recommend(input.category, input.bestPlatform, input.totalDuration);
        ...
        const audienceMatch = Math.min(100, Math.round((input.audienceSize === 'massive' ? 85 : input.audienceSize === 'large' ? 70 : 55) * 0.5 +
            seo.seoScore * 0.25 + hook.attentionScore * 0.25));
        // ── Analytics dashboard ──
        const analytics = CreatorAnalyticsEngine.calculate(seo.seoScore, thumbnail.overallScore, hook.attentionScore, retention.estimatedRetention, engagement.engagementScore, audienceMatch, publishing.readinessScore);
        // ── Overall creator score ──
        const creatorScore = analytics.overallCreatorScore;
        // ── Confidence ──
        const confidence = Math.min(100, Math.round(seo.seoScore * 0.15 + hook.attentionScore * 0.2 + retention.estimatedRetention * 0.2 +
...
//# sourceMappingURL=creator-planner.js.map