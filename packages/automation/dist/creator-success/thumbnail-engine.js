export class ThumbnailEngine {
    static analyze(hasText, hasSubject, hasContrast) {
        ...
        if (!hasSubject)
            improvements.push('Ensure main subject fills 60%+ of frame');
        if (!hasContrast)
            improvements.push('Increase contrast between subject and background');
