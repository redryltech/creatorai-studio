export class HookEngine {
    static analyze(hookText, totalDuration) {
        ...
        const improvements = [];
        if (!hasQuestion)
            improvements.push('Start with a question to create curiosity');
        if (!hasBoldClaim)
            improvements.push('Add a bold claim or surprising statement');
        if (!hasNumber)
            improvements.push('Include a number for specificity');
        if (wordCount > 20)
            improvements.push('Shorten hook — first 2 seconds decide everything');
