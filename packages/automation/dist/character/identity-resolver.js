...
export class IdentityResolver {
     ...
     */
    resolve(storyboard, directorPlan) {
        const allText = storyboard.frames.map((f) => `${f.frameDescription} ${f.narrationText} ${f.composition.mainSubject} ${f.composition.foreground} ${f.composition.midground} ${f.assets.vehicles.join(' ')} ${f.assets.characters.join(' ')} ${f.assets.props.join(' ')}`).join(' ').toLowerCase();
        const entities = [];
    ...
    // ── Builders ───────────────────────────────────────────
    buildEntity(id, displayName, category, priority, visibility, importance, continuityLevel, vehicleProfile, characterProfile, environmentProfile, scenePresence) {
    ...
    buildIdentityBlock(id, name, category, vehicle, character) {
        const lines = [`ENTITY ${id} (${category.toUpperCase()})`];
        lines.push(`Always use: ${name}`);
        if (vehicle) {
            lines.push(`Color: ${vehicle.primaryColor} ${vehicle.paintFinish}`);
            if (vehicle.secondaryColor && vehicle.secondaryColor !== 'Black')
                lines.push(`Secondary: ${vehicle.secondaryColor}`);
        ...
        if (character) {
            ...
            if (character.helmet)
                lines.push(`Helmet: ${character.helmet}`);
            if (character.accessories.length > 0)
                lines.push(`Accessories: ${character.accessories.join(', ')}`);
        }
        lines.push(`Maintain identity across ALL scenes.`);
        return lines.join('\n');
    }
    buildAppearanceMemory(name, category, vehicle, character) {
        const preferredColors = [];
        const requiredObjects = [];
        const requiredClothing = [];
        const requiredVehicleParts = [];
        const forbiddenChanges = [];
        if (vehicle) {
            preferredColors.push(vehicle.primaryColor);
            if (vehicle.secondaryColor)
                preferredColors.push(vehicle.secondaryColor);
        ...
        if (character) {
            ...
            if (character.helmet)
                requiredObjects.push(character.helmet);
    ...
    buildCharacterProfile(gender, isRider, isAthlete, directorPlan) {
        return {
            gender,
            ageRange: '25-35',
            height: 'average',
