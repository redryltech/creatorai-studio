...
export class ContinuityEngine {
     ...
     */
    static analyze(storyboard, entities, seedManager) {
        const issues = [];
        const entityScores = {};
        // Initialize scores
            ...
            for (const entity of entities) {
                ...
                if (entity.vehicleProfile && entity.category === 'vehicle') {
                    ...
                    if (prev && entity.scenePresence.includes(prev.sceneId)) {
                        ...
                        const wrongColors = ['red', 'blue', 'white', 'silver', 'orange', 'yellow'].filter((c) => c !== vp.primaryColor.toLowerCase() && frameText.includes(c) && !prevText.includes(c));
                ...
                if (entity.characterProfile && entity.category === 'human') {
                    ...
                    if (prev && entity.scenePresence.includes(prev.sceneId)) {
                        ...
                        const contradictions = ['suit', 'dress', 'shorts', 'swimwear', 'pajamas'].filter((c) => frameText.includes(c) && !cp.clothing.overall.toLowerCase().includes(c));
...
//# sourceMappingURL=continuity-engine.js.map