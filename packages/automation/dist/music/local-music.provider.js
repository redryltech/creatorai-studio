...
export class LocalMusicProvider {
    providerId = 'local_music';
    providerName = 'Local Music Library';
    priority = 0; // Highest — it's always available
    musicDir;
    indexPath;
    scanner;
    tracks = [];
    ...
    async searchTracks(request) {
        ...
        if (request.mood) {
            ...
            results = results.filter((t) => t.mood.toLowerCase().includes(mood) || t.tags.some((tag) => tag.includes(mood)));
        }
        ...
        if (request.tags?.length) {
            ...
            results = results.filter((t) => t.tags.some((tag) => searchTags.some((st) => tag.includes(st))));
        }
    ...
    async selectTrack(request) {
        ...
        const scored = candidates.map((track) => {
            let score = 50; // base
            // Category match
            if (track.category === bestCategory)
                score += 30;
            // Energy match from emotions
            if (request.emotions?.length) {
                const avgEnergy = request.emotions.reduce((sum, e) => sum + (EMOTION_ENERGY[e.toLowerCase()] ?? 5), 0) / request.emotions.length;
                const energyDiff = Math.abs(track.energy - avgEnergy);
                score += Math.max(0, 15 - energyDiff * 3);
            }
            // Tag match
            if (request.tags?.length) {
                const tagMatches = request.tags.filter((t) => track.tags.some((tt) => tt.includes(t.toLowerCase()))).length;
                score += tagMatches * 5;
            }
            // Mood match
    ...
    async validate(track) {
        ...
        try {
            ...
            if (duration < 5)
                return { valid: false, error: `Too short: ${duration}s` };
            return { valid: true };
        }
        catch (err) {
...
//# sourceMappingURL=local-music.provider.js.map