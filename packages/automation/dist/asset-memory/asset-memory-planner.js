...
export class AssetMemoryPlanner {
     ...
     */
    static plan(directorPlan, storyboard, charDb, sceneGraphPkg, worldStatePkg) {
        ...
        const promptTemplates = PROMPT_TEMPLATES.map((t, idx) => ({
            id: `tmpl_${idx + 1}`, name: t.name, category: t.category,
    ...
    // ── Recommendations ──
    static generateRecommendations(assets, dp) {
        const recs = [];
    ...
    // ── Helper ──
    static makeAsset(id, name, category, subcategory, projectOrigin, tags, data) {
        return {
...
//# sourceMappingURL=asset-memory-planner.js.map