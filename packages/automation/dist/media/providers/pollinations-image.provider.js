...
export class PollinationsImageProvider {
    ...
    providerName = 'Pollinations.ai (Flux)';
    mediaType = 'image';
    priority = 10; // After Replicate (0) and Gemini (5), before Mock (99)
    targetWidth;
    targetHeight;
    timeoutMs;
    maxRetries;
    ...
    async generate(request) {
        ...
        const requestedSeed = request.seed ?? (this.baseSeed + this.imageCounter);
        this.imageCounter++;
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                const result = await this.generateSingleImage(enhancedPrompt, width, height, requestedSeed, sceneId);
                if (result.success) {
     ...
     */
    enhancePrompt(rawPrompt, negativePrompt, style, width, height) {
        const parts = [];
     ...
     */
    async generateSingleImage(prompt, targetWidth, targetHeight, seed, sceneId) {
        // ── Build URL ──
    ...
    postProcessImage(inputPath, outputPath, targetWidth, targetHeight) {
        try {
            ...
            return {
                success: false,
                error: err.message?.slice(0, 200) ?? 'FFmpeg post-processing failed',
            };
        }
    }
    /**
     ...
     */
    validateImage(filePath, expectedWidth, expectedHeight) {
        // Check existence
        ...
        try {
            ...
            return { valid: false, error: `FFprobe validation failed: ${err.message?.slice(0, 100)}` };
        }
    }
    /**
