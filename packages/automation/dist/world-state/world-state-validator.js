...
export class WorldStateValidator {
    static validate(pkg) {
        ...
        if (pkg.snapshots.length === 0) {
            errors.push('No snapshots');
            score -= 50;
        }
        // Snapshot validation
        ...
        for (const snap of pkg.snapshots) {
            if (seenIds.has(snap.snapshotId)) {
                errors.push(`Duplicate snapshot: ${snap.snapshotId}`);
                score -= 10;
            }
            seenIds.add(snap.snapshotId);
            if (snap.timestamp < prevTimestamp) {
                errors.push(`Snapshot ${snap.snapshotId}: timestamp goes backward`);
                score -= 10;
            }
            prevTimestamp = snap.timestamp;
            if (!snap.sceneId) {
                errors.push(`Snapshot ${snap.snapshotId}: no sceneId`);
                score -= 5;
            }
            if (!snap.environment.location) {
                warnings.push(`Snapshot ${snap.snapshotId}: no location`);
                score -= 2;
            }
            // Check for impossible states
            if (snap.environment.rain > 0 && snap.environment.snow > 0 && snap.environment.rain + snap.environment.snow > 1.5) {
                warnings.push(`Snapshot ${snap.snapshotId}: heavy rain AND snow simultaneously`);
                score -= 3;
            }
        }
        // Transition validation
        for (const trans of pkg.transitions) {
            if (!trans.fromSceneId || !trans.toSceneId) {
                errors.push('Transition missing scene IDs');
                score -= 5;
            }
            if (trans.transitionDuration < 0) {
                errors.push('Negative transition duration');
                score -= 5;
            }
        }
        // Timeline validation
        if (pkg.timeline.totalDuration <= 0) {
...
//# sourceMappingURL=world-state-validator.js.map