...
export class WorldStatePlanner {
     ...
     */
    static plan(sceneGraphPkg, charDb, storyboard, directorPlan) {
        ...
        for (let i = 0; i < sceneGraphPkg.scenes.length; i++) {
            ...
            const snapshot = WorldStatePlanner.buildSnapshot(graph, charDb, dirScene, frame, cumulativeTime, i + 1, worldVersion);
    ...
    // Snapshot Builder
    // ══════════════════════════════════════════════════════════
    static buildSnapshot(graph, charDb, dirScene, frame, timestamp, sceneOrder, worldVersion) {
        const nodes = Object.values(graph.nodes);
        const env = (dirScene?.environment ?? 'studio');
        const weather = (dirScene?.weather ?? 'clear');
        const timeOfDay = (dirScene?.timeOfDay ?? 'afternoon');
        const lighting = (dirScene?.lighting ?? 'natural');
        // Environment state
        ...
        // Character states
        const characters = charDb.entities
        ...
        // Vehicle states
        const vehicles = charDb.entities
            ...
            return {
                ...
                indicators: 'none', damage: 0, dirt: 0, fuel: 0.8,
            };
        });
    ...
    // State Transition
    // ══════════════════════════════════════════════════════════
    static computeTransition(from, to) {
        const envDelta = {};
        for (const key of Object.keys(from.environment)) {
            if (from.environment[key] !== to.environment[key]) {
                envDelta[key] = to.environment[key];
            }
        }
        const lightDelta = {};
        if (from.lighting.temperature !== to.lighting.temperature)
            lightDelta.temperature = to.lighting.temperature;
        if (from.lighting.intensity !== to.lighting.intensity)
            lightDelta.intensity = to.lighting.intensity;
        if (from.lighting.contrast !== to.lighting.contrast)
            lightDelta.contrast = to.lighting.contrast;
        const camDelta = {};
        if (from.camera.lens !== to.camera.lens)
            camDelta.lens = to.camera.lens;
        if (from.camera.fov !== to.camera.fov)
            camDelta.fov = to.camera.fov;
        if (from.camera.movement !== to.camera.movement)
            camDelta.movement = to.camera.movement;
        const charDeltas = to.characters.map((tc) => {
            const fc = from.characters.find((c) => c.entityId === tc.entityId);
            if (!fc)
                return { entityId: tc.entityId, changes: { appeared: true } };
            const changes = {};
            if (fc.pose !== tc.pose)
                changes.pose = tc.pose;
            if (fc.clothing !== tc.clothing)
                changes.clothing = tc.clothing;
            if (fc.expression !== tc.expression)
                changes.expression = tc.expression;
        ...
        const vehicleDeltas = to.vehicles.map((tv) => {
            ...
            if (!fv)
                return { entityId: tv.entityId, changes: { appeared: true } };
            const changes = {};
            if (fv.speed !== tv.speed)
                changes.speed = tv.speed;
            if (fv.headlights !== tv.headlights)
                changes.headlights = tv.headlights;
    ...
    // ══════════════════════════════════════════════════════════
    static detectIssues(snapshots, transitions, charDb) {
        const issues = [];
        let issueCounter = 0;
        for (let i = 0; i < transitions.length; i++) {
            ...
            if (from.environment.rain === 0 && to.environment.rain > 0.5) {
                issues.push(WorldStatePlanner.makeIssue(++issueCounter, 'unexpected_weather', 'warning', from.sceneId, to.sceneId, null, `Sudden rain start between scene ${from.sceneOrder} and ${to.sceneOrder}`, 'Add a transitional shot showing clouds gathering', 'Add rain gradually'));
            }
            ...
            if (elevDiff > 40 && trans.transitionType !== 'jump') {
                issues.push(WorldStatePlanner.makeIssue(++issueCounter, 'unexpected_lighting', 'info', from.sceneId, to.sceneId, null, `Large sun position change (${elevDiff.toFixed(0)}°) suggesting time jump`, 'Add time-lapse transition or adjust lighting', 'Gradual lighting shift'));
            }
            // Character teleportation
            for (const cDelta of trans.characterDeltas) {
                if (cDelta.changes.appeared)
                    continue;
                const fc = from.characters.find((c) => c.entityId === cDelta.entityId);
                const tc = to.characters.find((c) => c.entityId === cDelta.entityId);
                if (fc && tc) {
                    const dist = Math.sqrt((fc.position.x - tc.position.x) ** 2 +
                        (fc.position.y - tc.position.y) ** 2 +
                        (fc.position.z - tc.position.z) ** 2);
                    if (dist > 20 && trans.transitionDuration < 2) {
                        issues.push(WorldStatePlanner.makeIssue(++issueCounter, 'character_teleportation', 'warning', from.sceneId, to.sceneId, cDelta.entityId, `${fc.displayName} moved ${dist.toFixed(1)}m in ${trans.transitionDuration}s`, 'Add movement transition or cut', 'Use cross-dissolve'));
                    }
                }
                // Costume change
                if (cDelta.changes.clothing) {
                    issues.push(WorldStatePlanner.makeIssue(++issueCounter, 'costume_change', 'warning', from.sceneId, to.sceneId, cDelta.entityId, `Clothing changed between scenes`, 'Maintain consistent wardrobe', 'Keep same outfit'));
                }
            }
            // Vehicle teleportation
            for (const vDelta of trans.vehicleDeltas) {
                if (vDelta.changes.appeared)
                    continue;
                const fv = from.vehicles.find((v) => v.entityId === vDelta.entityId);
                const tv = to.vehicles.find((v) => v.entityId === vDelta.entityId);
                if (fv && tv) {
                    const dist = Math.sqrt((fv.position.x - tv.position.x) ** 2 + (fv.position.z - tv.position.z) ** 2);
                    if (dist > 50 && trans.transitionDuration < 3) {
                        issues.push(WorldStatePlanner.makeIssue(++issueCounter, 'vehicle_teleportation', 'warning', from.sceneId, to.sceneId, vDelta.entityId, `Vehicle moved ${dist.toFixed(0)}m instantly`, 'Add driving transition', 'Use tracking shot'));
                    }
                }
            }
            // Missing props
            ...
            for (const prop of missingProps) {
                issues.push(WorldStatePlanner.makeIssue(++issueCounter, 'missing_props', 'info', from.sceneId, to.sceneId, null, `Prop "${prop}" was in scene ${from.sceneOrder} but missing in ${to.sceneOrder}`, 'Add prop to scene or explain removal', 'Include in prompt'));
            }
            ...
            if (camXDiff > 8 && from.environment.location === to.environment.location) {
                issues.push(WorldStatePlanner.makeIssue(++issueCounter, '180_degree_rule', 'info', from.sceneId, to.sceneId, null, 'Camera crossed the action axis — may confuse spatial orientation', 'Keep camera on same side of action line', 'Add cutaway between angles'));
            }
        }
        return issues;
    }
    ...
    // Metrics
    // ══════════════════════════════════════════════════════════
    static computeMetrics(snapshots, transitions, issues) {
        ...
        const overallProductionScore = Math.round((continuityScore * 0.3 + environmentScore * 0.15 + cameraScore * 0.15 +
    ...
    // ── Helpers ──
    static makeIssue(num, type, severity, from, to, entityId, description, suggestion, transitionSuggestion) {
        return {
...
//# sourceMappingURL=world-state-planner.js.map