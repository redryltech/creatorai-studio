...
export class PromptAssembler {
     ...
     */
    static assemble(frame, dirScene, charDb, worldSnap, brandKit, styleGuide) {
        const blocks = [];
        let blockId = 0;
        const bk = (type, content, priority) => {
            if (!content.trim())
                return;
        ...
        // ── Character Block ──
        const sceneEntities = charDb.entities.filter((e) => e.scenePresence.includes(frame.sceneId) && (e.category === 'human' || e.category === 'animal'));
        if (sceneEntities.length > 0) {
            const charLines = sceneEntities.map((e) => e.identityBlock.split('\n').slice(1).join(', ')).join('; ');
            bk('character', charLines, 8);
        }
        // ── Vehicle Block ──
        const vehicleEntities = charDb.entities.filter((e) => e.scenePresence.includes(frame.sceneId) && e.category === 'vehicle');
        ...
        if (contParts.length > 0)
            bk('continuity', contParts.join(', '), 6);
        return blocks;
    }
}
