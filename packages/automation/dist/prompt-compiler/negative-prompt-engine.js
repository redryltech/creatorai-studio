...
export class NegativePromptEngine {
     ...
     */
    static build(sceneId, charDb, currentVehicleColor) {
        const forbiddenObjects = [...BASE_OBJECTS];
        const forbiddenColors = [];
        const forbiddenClothing = [];
        const forbiddenVehicles = [];
        // Entity-specific negatives from character database
        for (const entity of charDb.entities) {
            if (!entity.scenePresence.includes(sceneId))
                continue;
     ...
     */
    static forProvider(spec, providerId) {
...
//# sourceMappingURL=negative-prompt-engine.js.map