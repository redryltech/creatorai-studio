...
export class TranslationPlanner {
     ...
     */
    static async translate(scenes, title, targetLanguages, sourceLanguage = 'en', geminiKey) {
        ...
        for (const lang of targetLanguages) {
            ...
            if (!langInfo)
                continue;
            try {
                const translated = await TranslationPlanner.translateToLanguage(scenes, title, lang, langInfo, geminiKey);
                translations.push(translated);
                log.info('Language translated', { language: langInfo.name, scenes: translated.scenes.length });
            }
            catch (err) {
                log.warn('Translation failed for language', { language: langInfo.name, error: err.message?.slice(0, 80) });
            }
        }
     ...
     */
    static async translateToLanguage(scenes, title, lang, langInfo, geminiKey) {
...
Return JSON: {"title": "translated title", "scenes": ["scene 1 translation", "scene 2 translation", ...], "hashtags": ["#tag1", "#tag2", "#tag3"]}`;
        try {
            ...
            log.warn('Gemini translation failed, using placeholder', { lang, error: err.message?.slice(0, 60) });
            return TranslationPlanner.buildPlaceholder(scenes, title, lang, langInfo);
        }
    }
    /**
     ...
     */
    static buildPlaceholder(scenes, title, lang, langInfo) {
        return {
            language: lang,
            languageName: langInfo.name,
            scenes: scenes.map((s) => ({
    ...
    static getIndianLanguages() {
        return Object.values(LANGUAGES).filter((l) => l.region === 'India');
    }
}
//# sourceMappingURL=translation-planner.js.map