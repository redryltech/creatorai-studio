...
export class DirectorValidator {
    ...
    static validate(plan) {
        ...
        if (!plan.globalStyle) {
            warnings.push('No global style set');
            score -= 5;
        }
        if (!plan.globalColorGrading) {
            warnings.push('No global color grading');
            score -= 5;
        }
        if (plan.colorPalette.length === 0) {
            warnings.push('Empty color palette');
            score -= 3;
        }
        // ── Duration check ──
        const totalDuration = plan.scenes.reduce((s, sc) => s + sc.sceneDuration, 0);
        if (totalDuration < 10) {
            errors.push(`Total duration too short: ${totalDuration}s`);
            score -= 20;
        }
        if (totalDuration > 180) {
            warnings.push(`Total duration very long: ${totalDuration}s`);
            score -= 5;
        }
        // ── Scene-level checks ──
        ...
        for (const scene of plan.scenes) {
            ...
            // Required fields
            if (!scene.sceneGoal) {
                warnings.push(`Scene ${scene.sceneOrder}: no goal`);
                score -= 2;
            }
            if (!scene.cameraStyle) {
                errors.push(`Scene ${scene.sceneOrder}: no camera style`);
                score -= 5;
            }
            if (!scene.lens) {
                errors.push(`Scene ${scene.sceneOrder}: no lens`);
                score -= 5;
            }
            if (!scene.lighting) {
                errors.push(`Scene ${scene.sceneOrder}: no lighting`);
                score -= 5;
            }
            if (!scene.environment) {
                errors.push(`Scene ${scene.sceneOrder}: no environment`);
                score -= 5;
            }
            if (!scene.colorGrading) {
                errors.push(`Scene ${scene.sceneOrder}: no color grading`);
                score -= 5;
            }
            if (scene.sceneDuration <= 0) {
                errors.push(`Scene ${scene.sceneOrder}: invalid duration`);
                score -= 10;
            }
            if (!scene.narration) {
                warnings.push(`Scene ${scene.sceneOrder}: no narration`);
                score -= 2;
            }
    ...
    static validateScene(scene) {
        ...
        if (!scene.cameraStyle)
            errors.push('Missing cameraStyle');
        if (!scene.lens)
            errors.push('Missing lens');
        if (!scene.lighting)
...
//# sourceMappingURL=director-validator.js.map