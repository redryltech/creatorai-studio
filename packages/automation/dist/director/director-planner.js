...
export class DirectorPlanner {
    ...
    static plan(script, title) {
        ...
        const scenes = script.scenes.map((scene, index) => DirectorPlanner.planScene(scene, index, script.scenes.length, profile, category));
    ...
    // ── Scene Planner ──────────────────────────────────────
    static planScene(scene, index, totalScenes, profile, category) {
    ...
    // ── Helpers ────────────────────────────────────────────
    static detectCategory(text, script) {
    ...
    static determineImportance(index, total) {
        if (index === 0)
            return 'hook';
        if (index === total - 1)
            return 'cta';
    ...
    static selectCameraMovement(camera, emotion, importance) {
        // Importance-based overrides
        if (importance === 'hook')
            return 'dolly_in';
        if (importance === 'cta')
            return 'pull_back';
        if (importance === 'climax')
            return 'crane_up';
    ...
    static selectSubjectPosition(importance, camera) {
        if (importance === 'hook')
            return 'center';
        if (camera === 'drone')
            return 'center';
        if (camera === 'macro')
            return 'full_frame';
        const positions = ['center', 'left_third', 'right_third', 'center', 'foreground'];
        return positions[Math.floor(Math.random() * positions.length)];
    }
    static selectWeather(emotion) {
        const map = {
    ...
    static selectMotionIntensity(importance, emotion, pacing) {
        if (importance === 'climax')
            return 'dynamic';
        if (pacing === 'fast')
            return emotion === 'excitement' ? 'extreme' : 'dynamic';
        if (pacing === 'slow')
            return 'subtle';
        return 'moderate';
    }
    static detectEnvironment(visualNotes) {
    ...
    static inferGoal(scene, importance) {
        const goals = {
    ...
    static harmonizeTransitions(scenes) {
    ...
    static selectThumbnail(scenes) {
        ...
        for (let i = 0; i < scenes.length; i++) {
            ...
            let score = 0;
            // Prefer hero shots
            if (scene.cameraStyle === 'hero_shot')
                score += 30;
            // Prefer dramatic lighting
            if (['dramatic', 'golden_hour', 'rim_light'].includes(scene.lighting))
                score += 20;
            // Prefer hook or climax
            if (scene.sceneImportance === 'hook')
                score += 15;
            if (scene.sceneImportance === 'climax')
                score += 25;
            // Prefer wide/medium shots
            if (['24mm', '35mm'].includes(scene.lens))
                score += 10;
            // Prefer visual effects
            score += scene.visualEffects.length * 5;
            // Penalize CTA scenes
            if (scene.sceneImportance === 'cta')
                score -= 20;
            if (score > bestScore) {
                bestScore = score;
                bestIdx = i;
    ...
    static buildConsistencyNotes(scenes, profile) {
        return [
    ...
    static extractCharacterDescription(script) {
        const text = script.fullNarration.toLowerCase();
        if (text.includes('rider') || text.includes('biker'))
            return 'motorcycle rider in full gear';
        if (text.includes('athlete') || text.includes('runner'))
            return 'athletic person in sportswear';
        if (text.includes('person') || text.includes('someone'))
            return 'determined individual';
        return 'subject appropriate to the scene';
    }
    static findRecurringElements(script) {
        const elements = [];
        const text = script.fullNarration.toLowerCase();
        if (text.includes('success') || text.includes('win'))
            elements.push('victory/triumph imagery');
        if (text.includes('journey') || text.includes('path'))
            elements.push('road/path symbolism');
        if (text.includes('light') || text.includes('fire'))
            elements.push('light/fire metaphor');
        if (text.includes('strength') || text.includes('power'))
            elements.push('strength imagery');
        return elements.length > 0 ? elements : ['consistent visual tone', 'matching color palette'];
    }
    static getColorPalette(grading) {
        const palettes = {
...
//# sourceMappingURL=director-planner.js.map