...
export class TrendAnalyzer {
    ...
    static analyze(topic, category) {
        ...
        const signals = [];
        const platforms = ['youtube_shorts', 'instagram_reels', 'tiktok', 'youtube', 'facebook', 'linkedin', 'x'];
        for (const platform of platforms) {
            const platformScore = affinity[platform] ?? 40;
            if (platformScore < 30)
                continue; // Skip irrelevant platforms
            // Viral keyword boost
            ...
            const velocity = viralBoost > 15 ? 'viral' :
                viralBoost > 8 ? 'rising' :
        ...
        const bestPlatform = (sortedByScore[0]?.platform ?? 'youtube_shorts');
