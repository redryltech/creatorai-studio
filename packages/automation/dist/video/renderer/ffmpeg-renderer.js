 ...
 */
export async function renderWithFFmpeg(timeline, captions, options, onProgress, isCancelled) {
    ...
    if (hasImages) {
        ...
        for (let i = 0; i < imagePaths.length; i++) {
            ...
            filterParts.push(`[${i}:v]scale=${width}:${height}:force_original_aspect_ratio=decrease,` +
        ...
        args.push('-f', 'lavfi', '-i', `color=c=0x111111:s=${width}x${height}:d=${totalDurationSec}:r=${options.fps}`, '-f', 'lavfi', '-i', `anullsrc=r=44100:cl=stereo`, '-t', String(totalDurationSec));
    }
    // Output settings
    args.push('-c:v', 'libx264', '-preset', 'fast', '-crf', '23', '-c:a', 'aac', '-b:a', '128k', '-r', String(options.fps), '-movflags', '+faststart', '-shortest', outputPath);
...
async function downloadFile(url, destPath) {
    ...
    try {
        ...
        log.warn('Asset download error', { url: url.slice(0, 80), error: err.message });
    }
}
//# sourceMappingURL=ffmpeg-renderer.js.map