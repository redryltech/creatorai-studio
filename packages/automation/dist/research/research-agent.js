...
export class ResearchAgent {
    agentId = 'automation.research';
    agentName = 'Research Agent';
    stage = AutomationStage.RESEARCH;
    validate(input) {
        const errors = [];
    ...
    async execute(input, onProgress, cancellation) {
        ...
        const llm = await ProviderRegistry.getInstance().getPrimary('llm');
        if (!llm)
            throw new Error('No LLM provider available for research');
        ...
        for (const provider of providers) {
            try {
                ...
                log.warn('Research provider failed', { providerId: provider.providerId, error: error.message });
            }
        }
...
Provide a comprehensive research report as JSON:
{
  ...
  "scores": {"trendScore":0,"difficultyScore":0,"opportunityScore":0,"confidenceScore":0}
        ...
        const report = {
            ...
            audience: parsed.audience ?? { primaryAge: '18-35', interests: [], painPoints: [], contentPreferences: [], activePlatforms: [], peakActivityHours: [], languagePreferences: [request.language] },
            topAngles: parsed.topAngles ?? [],
            contentGaps: parsed.contentGaps ?? [],
            references: parsed.references ?? [],
            scores: {
                trendScore: parsed.scores?.trendScore ?? 50,
                difficultyScore: parsed.scores?.difficultyScore ?? 50,
                opportunityScore: parsed.scores?.opportunityScore ?? 50,
                confidenceScore: parsed.scores?.confidenceScore ?? 70,
            },
            metadata: {
