...
export class InstagramPublisher {
    ...
    async refreshAuth(account) {
        ...
        const resp = await fetch(`${this.graphUrl}/oauth/access_token?` +
            `grant_type=fb_exchange_token` +
            `&client_id=${this.config.appId}` +
            `&client_secret=${this.config.appSecret}` +
            `&fb_exchange_token=${account.accessToken}`, { signal: AbortSignal.timeout(10000) });
        if (!resp.ok) {
    ...
    validate(request) {
        ...
        if (request.seo.description && request.seo.description.length > 2200)
            errors.push('Caption must be ≤2200 characters');
        if (request.seo.hashtags && request.seo.hashtags.length > 30)
            errors.push('Maximum 30 hashtags');
    ...
    async upload(request, account, onProgress) {
        ...
        for (let i = 0; i < MAX_POLL_ATTEMPTS; i++) {
            ...
            try {
                const statusResp = await fetch(`${this.graphUrl}/${containerId}?fields=status_code,status&access_token=${currentAccount.accessToken}`, { signal: AbortSignal.timeout(10000) });
                if (!statusResp.ok)
                    continue;
        ...
        if (!mediaId)
            throw new Error('No media ID returned after publishing');
        ...
        try {
            const mediaResp = await fetch(`${this.graphUrl}/${mediaId}?fields=permalink,thumbnail_url,timestamp,media_type&access_token=${currentAccount.accessToken}`, { signal: AbortSignal.timeout(10000) });
            if (mediaResp.ok) {
    ...
    async getStatus(platformPostId, account) {
        try {
            const resp = await fetch(`${this.graphUrl}/${platformPostId}?fields=permalink,media_type&access_token=${account.accessToken}`, { signal: AbortSignal.timeout(10000) });
            if (!resp.ok)
