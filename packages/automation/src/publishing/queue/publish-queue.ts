...
import type { PublishJob, PublishRequest, PublishResult, SocialAccount, SocialPlatformId } from '../types/publishing.types';

const log = Logger.for('PublishQueue');

export class PublishQueue {
  private static instance: PublishQueue | null = null;
  ...
  static getInstance(): PublishQueue { if (!PublishQueue.instance) PublishQueue.instance = new PublishQueue(); return PublishQueue.instance; }
  static resetInstance(): void { PublishQueue.instance = null; }
  ...
  enqueue(request: PublishRequest, userId: string, projectId: string, workflowId: string, priority: number = 10): PublishJob {
  ...
  /** Get a job by ID. */
  getJob(jobId: string): PublishJob | undefined { return this.jobs.get(jobId); }
  ...
  get pendingCount(): number { return Array.from(this.jobs.values()).filter((j) => j.status === 'queued').length; }
  get activeCount(): number { return Array.from(this.jobs.values()).filter((j) => j.status === 'uploading' || j.status === 'processing').length; }
  ...
  private async startProcessing(): Promise<void> {
    ...
    while (true) {
      ...
      if (pending.length === 0) { this.processing = false; return; }
      if (this.activeCount >= this.maxConcurrency) { await sleep(1000); continue; }

      const job = pending[0]!;
      this.processJob(job).catch((err) => log.error('Publish job error', { jobId: job.id }, err as Error));
    }
  }
