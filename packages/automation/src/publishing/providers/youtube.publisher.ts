...
export class YouTubePublisher implements IPublisher {
  readonly platformId: SocialPlatformId = 'youtube';
  readonly platformName = 'YouTube';

  private config: YouTubePublisherConfig | null = null;
  ...
  async authenticate(account: SocialAccount): Promise<boolean> {
    try {
      ...
      log.error('YouTube auth check failed', {}, error as Error);
      return false;
    }
  }

  async refreshAuth(account: SocialAccount): Promise<SocialAccount> {
    if (!this.config) throw new Error('YouTube publisher not configured — set YOUTUBE_CLIENT_ID/SECRET');
  ...
  validate(request: PublishRequest): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    if (!request.videoUrl) errors.push('Video URL or file path required');
    if (!request.seo.title) errors.push('Title is required');
    if (request.seo.title && request.seo.title.length > 100) errors.push('Title must be ≤100 characters');
    if (request.seo.description && request.seo.description.length > 5000) errors.push('Description must be ≤5000 characters');
    if (request.seo.tags && request.seo.tags.join(',').length > 500) errors.push('Total tag length must be ≤500 characters');
  ...
  async upload(
    ...
    if (request.videoUrl.startsWith('/') || request.videoUrl.startsWith('file://')) {
      ...
      if (!downloadResp.ok) throw new Error(`Failed to download video: ${downloadResp.status}`);
    ...
    const snippet: Record<string, unknown> = {
      ...
      categoryId: (request.options as Record<string, string>)?.categoryId ?? '22', // People & Blogs
      defaultLanguage: 'en',
    };
    ...
    const initResp = await fetch(
      `${API_UPLOAD}/videos?uploadType=resumable&part=snippet,status`,
      {
        method: 'POST',
        headers: {
    ...
    const uploadResult = (await uploadResp.json()) as YouTubeVideoResource;
    const videoId = uploadResult.id;

    if (!videoId) throw new Error('YouTube upload completed but no video ID returned');
    ...
    if (request.thumbnailUrl) {
      ...
      try {
        ...
        const thumbResp = await fetch(
          `${API_UPLOAD}/thumbnails/set?videoId=${videoId}`,
          {
            method: 'POST',
            headers: {
              Authorization: `Bearer ${currentAccount.accessToken}`,
        ...
        log.warn('Thumbnail upload error (non-critical)', {}, err as Error);
      }
    }
    ...
    for (let i = 0; i < maxChecks; i++) {
      ...
      try {
        const statusResp = await fetch(
          `${API_DATA}/videos?part=status,processingDetails&id=${videoId}`,
          { headers: { Authorization: `Bearer ${currentAccount.accessToken}` }, signal: AbortSignal.timeout(10000) },
        );
        if (statusResp.ok) {
          const statusData = (await statusResp.json()) as { items?: Array<{ status?: { uploadStatus?: string } }> };
          processingStatus = statusData.items?.[0]?.status?.uploadStatus ?? processingStatus;
          if (processingStatus === 'processed') break;
        }
      } catch { /* continue */ }
    }
  ...
  async schedule(request: PublishRequest, account: SocialAccount, scheduleAt: Date): Promise<PublishResult> {
    return this.upload(
      { ...request, scheduleAt: scheduleAt.toISOString(), visibility: 'private' },
      account,
      () => {},
    );
  }
  ...
  async update(platformPostId: string, updates: Partial<PublishRequest['seo']>, account: SocialAccount): Promise<boolean> {
    ...
    const body: Record<string, unknown> = { id: platformPostId, snippet: {} };
    if (updates.title) (body.snippet as Record<string, unknown>).title = updates.title;
    if (updates.description) (body.snippet as Record<string, unknown>).description = updates.description;
    if (updates.tags) (body.snippet as Record<string, unknown>).tags = updates.tags;
  ...
  async getStatus(platformPostId: string, account: SocialAccount): Promise<{ status: string; url: string }> {
    ...
    if (!resp.ok) return { status: 'unknown', url: `https://youtube.com/watch?v=${platformPostId}` };

    const data = (await resp.json()) as { items?: Array<{ status?: { privacyStatus?: string; uploadStatus?: string } }> };
  ...
  private buildDescription(request: PublishRequest): string {
