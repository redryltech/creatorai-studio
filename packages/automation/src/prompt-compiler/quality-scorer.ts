...
import type { CanonicalPrompt, PromptBlock, PromptQualityScore } from './prompt.types';

export class QualityScorer {
  /**
   * Score the quality of a compiled canonical prompt.
   */
  static score(prompt: CanonicalPrompt): PromptQualityScore {
    const blocks = prompt.blocks;
    const blockTypes = new Set(blocks.map((b) => b.type));

    const hasBlock = (type: string) => blockTypes.has(type as PromptBlock['type']);
    const blockContent = (type: string) => blocks.find((b) => b.type === type)?.content.length ?? 0;
    ...
    const providerReadiness = Math.min(100,
      (prompt.masterPrompt.length > 100 ? 40 : 20) +
    ...
    const overallScore = Math.round(
      completeness * 0.25 + characterQuality * 0.1 + sceneQuality * 0.15 +
