...
import type { CanonicalPrompt, ProviderPrompt, ProviderSettings, NegativePromptSpec, PromptLength } from './prompt.types';
...
export class ProviderCompiler {
  ...
  static compileAll(
    ...
    negSpec: NegativePromptSpec,
    dirScene: DirectorScenePlan | undefined,
    seed: number,
    promptLength: PromptLength,
  ): ProviderPrompt[] {
    return PROVIDERS.map((provider) =>
      ProviderCompiler.compileForProvider(canonical, negSpec, dirScene, seed, provider, promptLength),
    );
  }
  ...
  static compileForProvider(
    ...
    if (provider.promptPrefix) prompt = provider.promptPrefix + prompt;
  ...
  static getProviders(): ProviderDef[] { return [...PROVIDERS]; }

  /** Get a specific provider definition. */
  static getProvider(id: string): ProviderDef | undefined { return PROVIDERS.find((p) => p.id === id); }
}
