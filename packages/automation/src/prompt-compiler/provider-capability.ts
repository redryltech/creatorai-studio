export interface ProviderCapability {
  maxDuration: number; maxResolution: string; supportsNegative: boolean;
  supportsSeed: boolean; supportsCamera: boolean; maxTokens: number;
  supportedStyles: string[]; costPerUnit: number;
}

export class ProviderCapabilityMap {
  private static capabilities: Record<string, ProviderCapability> = {
  ...
  static get(providerId: string): ProviderCapability | undefined { return ProviderCapabilityMap.capabilities[providerId]; }
  static getAll(): Record<string, ProviderCapability> { return { ...ProviderCapabilityMap.capabilities }; }
}
