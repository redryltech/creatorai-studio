...
export class ProviderOptimizer {
  static optimize(prompt: string, providerId: string): { optimizedPrompt: string; tokenCount: number; truncated: boolean; safetyClean: boolean } {
    const cap = ProviderCapabilityMap.get(providerId);
    if (!cap) return { optimizedPrompt: prompt, tokenCount: prompt.split(/\s+/).length, truncated: false, safetyClean: true };
    ...
    for (const u of unsafe) { if (safePrompt.toLowerCase().includes(u)) { safePrompt = safePrompt.replace(new RegExp(u, 'gi'), '[removed]'); safetyClean = false; } }
