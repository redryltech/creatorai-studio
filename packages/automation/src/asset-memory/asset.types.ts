...
export type AssetCategory =
  | 'character' | 'vehicle' | 'animal' | 'product' | 'building'
  | 'prop' | 'weapon' | 'logo' | 'icon' | 'font' | 'music'
  | 'voice' | 'sound_effect' | 'environment' | 'transition'
  | 'intro' | 'outro' | 'thumbnail_template' | 'overlay_template'
  | 'brand_element' | 'prompt_template';
...
export interface BrandKit {
  ...
  logoMeta: { name: string; variants: string[]; placement: string };
  watermark: string;
...
export type PromptTemplateCategory =
  | 'commercial' | 'cinematic' | 'documentary' | 'luxury' | 'travel'
  | 'sports' | 'tech' | 'education' | 'motivational' | 'product_review'
  | 'podcast' | 'youtube_shorts';
...
export interface PromptTemplate {
  ...
  styleSuffix: string;
  variables: string[];        // e.g. ["{subject}", "{lighting}"]
  usageCount: number;
  createdAt: string;
}
...
export interface EmbeddingRecord {
  ...
  placeholder: string;          // e.g. "[EMBED:char_001]"
...
// ════════════════════════════════════════════════════════════
// Asset Memory Package — the output
// ════════════════════════════════════════════════════════════
...
export interface AssetExportFormats {
  ...
  debugPackage: { totalAssets: number; categories: Record<string, number>; orphans: string[] };
}
