...
// ── SFX Synthesis Recipes ──
// Each recipe is an FFmpeg lavfi filter that produces a specific sound.
...
const SFX_LIBRARY: Record<string, SfxRecipe> = {
  // ── Vehicle sounds ──
  engine_start: {
...
const SCENE_SFX_MAP: Array<{ keywords: string[]; effects: string[]; ambient: string | null }> = [
...
export class SfxGenerator {
  ...
  static generate(recipeName: string, outputDir: string): SoundEffect | null {
    ...
    try {
      ...
      log.warn('SFX generation failed', { recipe: recipeName, error: (err as Error).message?.slice(0, 80) });
      return null;
    }
  }
   ...
   */
  static selectForScene(sceneText: string, sceneOrder: number): { effects: string[]; ambient: string | null; transition: string | null } {
    ...
    for (const rule of SCENE_SFX_MAP) {
      if (rule.keywords.some((kw) => lower.includes(kw))) {
        ...
        if (rule.ambient && !ambient) ambient = rule.ambient;
      }
    }
  ...
  static getLibrary(): string[] { return Object.keys(SFX_LIBRARY); }
