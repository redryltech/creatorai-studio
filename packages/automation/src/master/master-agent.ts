...
import type { CancellationToken, ProgressCallback } from '../interfaces/automation-agent.interface';
import type {
  AutomationRequest, AutomationPlan, AutomationResponse,
  ResearchReport, ContentPlan, ScriptPackage,
  WorkflowExecution, WorkflowTask, WorkflowMetrics,
...
export class MasterAgent {
  ...
  private readonly activeExecutions: Map<string, { cancel: () => void }> = new Map();
  ...
  async executeAutomation(
    ...
    let cancelled = false;
    const cancellation: CancellationToken = { get isCancelled() { return cancelled; } };
    ...
    this.executeWorkflow(plan, cancellation, userId).catch((error) => {
      log.error('Automation failed', { executionId }, error as Error);
    }).finally(() => {
  ...
  private resolveTaskInput(
    ...
    const input: Record<string, unknown> = { request: plan.request };
    ...
    const findOutput = (prefix: string) => {
    ...
    switch (agentId) {
      ...
      case 'automation.creator_success':
        ...
        const scriptOutput = findOutput(`script-${idx}`) as Record<string, unknown> | null;
        if (scriptOutput) {
          const pi = input.plannerInput as Record<string, unknown>;
          pi.hookText = (scriptOutput.hook as Record<string, unknown>)?.text ?? '';
          pi.fullNarration = scriptOutput.fullNarration ?? '';
          const scenes = scriptOutput.scenes as Array<Record<string, unknown>> | undefined;
          if (scenes) {
            pi.sceneDurations = scenes.map((s) => (s.duration as number) ?? 6);
            pi.totalDuration = (pi.sceneDurations as number[]).reduce((a: number, b: number) => a + b, 0);
          }
        }
        break;
    }
  ...
  private markTaskStatus(task: WorkflowTask, status: TaskStatus): void {
    task.status = status;
  }

  private createInitialMetrics(taskCount: number): WorkflowMetrics {
  ...
  private updateMetrics(workflow: WorkflowExecution): void {
  ...
  private emitEvent(userId: string, event: string, data: Record<string, unknown>): void {
    this.sseManager.sendToUser(userId, event, { ...data, timestamp: new Date().toISOString() });
  }

  private buildResponseMessage(request: AutomationRequest, plan: AutomationPlan): string {
    return [
