...
const CATEGORY_DEFAULTS: Record<string, { mood: string; energy: number; bpm: number; tags: string[] }> = {
...
export interface ScanResult {
  ...
  categories: string[];
  duplicates: Array<{ original: string; duplicate: string; hash: string }>;
...
export interface MusicIndex {
  ...
  categories: string[];
  tracks: MusicTrack[];
  fileHashes: Record<string, string>; // path → contentHash for change detection
}

export class MusicScanner {
  private musicDir: string;
  private indexPath: string;
  private cache: MusicIndex | null = null;
   ...
   */
  scan(forceRescan = false): ScanResult {
    ...
    const hashMap = new Map<string, string>(); // contentHash → trackId
    const duplicates: Array<{ original: string; duplicate: string; hash: string }> = [];
    ...
    for (const category of subdirs) {
      ...
      for (const filename of files) {
        ...
        try {
          ...
          errors.push({ file: filePath, error: (err as Error).message?.slice(0, 100) ?? 'Unknown' });
        }
      }
    }
  ...
  private isCacheValid(): boolean {
    ...
    try {
      ...
      if (currentFiles.length !== indexedPaths.size) return false;

      // Check each file still exists and size hasn't changed
      for (const track of index.tracks) {
        if (!existsSync(track.path)) return false;
        const stat = statSync(track.path);
        if (stat.size !== track.sizeBytes) return false;
      }
  ...
  private listAllAudioFiles(): string[] {
    const files: string[] = [];
    if (!existsSync(this.musicDir)) return files;
  ...
  private loadCachedIndex(): MusicIndex | null {
    try {
  ...
  private saveIndex(index: MusicIndex): void {
    try {
      writeFileSync(this.indexPath, JSON.stringify(index, null, 2));
    } catch (err) {
      log.error('Failed to save music index', {}, err as Error);
    }
  }
