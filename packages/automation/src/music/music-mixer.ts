 ...
 */
export const DEFAULT_MIX_CONFIG: MusicMixConfig = {
  enabled: true,
  autoSelect: true,
  randomTrack: false,
  loopMusic: true,
...
export class MusicMixer {
   ...
   */
  private static buildFFmpegCommand(input: MixInput): string[] {
    const { videoPath, voicePaths, voiceDurations, musicTrack, totalDuration, config, outputPath } = input;
    ...
    if (config.duckingEnabled) {
      ...
      filters.push(
        `[music_vol][voice]sidechaincompress=` +
  ...
  private static failResult(outputPath: string, error: string): MixResult {
    return {
