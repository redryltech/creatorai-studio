...
export class MusicAgent implements IAutomationAgent<MusicInput, MusicPackage> {
  readonly agentId = 'automation.music';
  readonly agentName = 'Music Agent';
  readonly stage = AutomationStage.MEDIA;

  validate(input: MusicInput): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    if (!input.scriptPackage) errors.push('ScriptPackage required');
    if (input.totalDuration <= 0) errors.push('Total duration must be positive');
    return { valid: errors.length === 0, errors };
  }

  estimateCost(_input: MusicInput): { costUsd: number; breakdown: string[] } {
  ...
  async execute(
    ...
    if (musicProvider) {
      try {
        ...
        log.warn('Music provider failed, falling back to LLM recommendation', {}, err as Error);
      }
    }
...
JSON: {"genre":"string","mood":"string","tempo":120,"style":"string","instruments":["string"],"energyLevel":"low|medium|high"}` }],
    ...
    try { parsed = JSON.parse(response.content); } catch { /* use defaults */ }

    onProgress(100, 'Music parameters selected');

    return {
      id: generateId(ID_PREFIXES.asset),
      genre: (parsed.genre as string) ?? 'cinematic',
      mood: (parsed.mood as string) ?? scriptPackage.metadata.tone,
      tempo: (parsed.tempo as number) ?? 120,
      duration: totalDuration,
