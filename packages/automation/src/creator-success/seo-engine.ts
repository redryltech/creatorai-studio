import type { SeoAnalysis } from './creator.types';
export class SeoEngine {
  static analyze(topic: string, title: string, description: string, keywords: string[]): SeoAnalysis {
    const titleLower = title.toLowerCase(); const topicLower = topic.toLowerCase();
    ...
    if (!titleHasKeyword) suggestions.push('Add primary keyword to title');
    if (title.length > 70) suggestions.push('Shorten title to under 70 characters');
    if (title.length < 20) suggestions.push('Title too short — add more descriptive words');
    if (!descHasKeyword) suggestions.push('Include primary keyword in description');
    if (keywords.length < 5) suggestions.push('Add more keywords for better discoverability');
    if (!description.includes('#')) suggestions.push('Add hashtags to description');
