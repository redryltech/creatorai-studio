...
const POSITION_TO_ROT: Record<string, StoryboardFrame['composition']['ruleOfThirdsPosition']> = {
...
const CAMERA_POSITION: Record<string, CameraInfo['position']> = {
...
const CAMERA_HEIGHT: Record<string, CameraInfo['height']> = {
...
const CAMERA_DISTANCE: Record<string, CameraInfo['distance']> = {
...
const LENS_FOV: Record<string, CameraInfo['fov']> = {
...
export class StoryboardPlanner {
  ...
  static plan(directorPlan: DirectorPlan): Storyboard {
    ...
    const frames: StoryboardFrame[] = directorPlan.scenes.map((scene, index) => {
      const frame = StoryboardPlanner.buildFrame(
        scene,
        index,
        directorPlan,
        cumulativeTime,
  ...
  private static buildComposition(scene: DirectorScenePlan): VisualComposition {
    ...
    const supportingObjects: string[] = [];
    ...
    for (const kw of objectKeywords) {
      if (envLower.includes(kw)) supportingObjects.push(kw);
    }

    // Depth layout from camera
    const depthMap: Record<string, VisualComposition['depthLayout']> = {
  ...
  private static buildCameraInfo(scene: DirectorScenePlan): CameraInfo {
    return {
  ...
  private static describeCameraPath(movement: string): string {
  ...
  private static buildMotionPlan(scene: DirectorScenePlan): MotionPlan {
    const motionSpeed: MotionPlan['motionSpeed'] =
      scene.motionIntensity === 'extreme' ? 'hyper' :
    ...
    if (scene.visualEffects.includes('dust_particles')) particleMotion = 'Dust particles drifting in light beams';
    else if (scene.visualEffects.includes('rain_effect')) particleMotion = 'Rain falling vertically with wind variance';
    else if (scene.visualEffects.includes('snow_effect')) particleMotion = 'Snowflakes drifting gently downward';
    else if (scene.visualEffects.includes('smoke_effect')) particleMotion = 'Smoke wisps curling and dissipating';
    else if (scene.visualEffects.includes('sparks')) particleMotion = 'Sparks flying outward with decay';
    else if (scene.visualEffects.includes('bokeh')) particleMotion = 'Bokeh light orbs floating in background';
  ...
  private static buildTiming(scene: DirectorScenePlan, startTime: number): FrameTiming {
    ...
    const curve: FrameTiming['animationCurve'] =
      scene.motionIntensity === 'extreme' ? 'spring' :
  ...
  private static buildAssets(scene: DirectorScenePlan, plan: DirectorPlan): AssetRequirements {
    const envDetails = (scene.environmentDetails + ' ' + scene.narration).toLowerCase();

    const characters: string[] = [];
    if (plan.characterDescription) characters.push(plan.characterDescription);
    if (envDetails.includes('rider') || envDetails.includes('person')) characters.push('Person/rider');

    const vehicles: string[] = [];
    const vehicleWords = ['motorcycle', 'bike', 'car', 'vehicle', 'ninja', 'kawasaki', 'truck', 'suv'];
    for (const vw of vehicleWords) {
      if (envDetails.includes(vw)) { vehicles.push(vw); break; }
    }

    const buildings: string[] = [];
    if (['city', 'night_city', 'cyberpunk'].includes(scene.environment)) buildings.push('Urban buildings/skyline');
    if (scene.environment === 'factory') buildings.push('Industrial structures');

    const envAssets: string[] = [];
    if (['forest', 'mountains', 'countryside'].includes(scene.environment)) envAssets.push('Natural landscape');
    if (['beach', 'ocean'].includes(scene.environment)) envAssets.push('Water/coast');
    if (['highway', 'race_track'].includes(scene.environment)) envAssets.push('Road/track surface');

    const props: string[] = [];
    if (envDetails.includes('helmet')) props.push('Helmet');
    if (envDetails.includes('exhaust')) props.push('Exhaust system');
    if (envDetails.includes('dashboard')) props.push('Instrument cluster');

    const soundEffects: string[] = [];
    if (vehicles.length > 0) soundEffects.push('Engine sound');
    if (scene.weather === 'rain' || scene.weather === 'heavy_rain') soundEffects.push('Rain ambience');
    if (scene.environment === 'city') soundEffects.push('Urban ambience');
    if (scene.motionStyle === 'driving' || scene.motionStyle === 'drifting') soundEffects.push('Tire/road sound');
  ...
  private static buildStyle(scene: DirectorScenePlan, plan: DirectorPlan): VisualStyle {
    return {
  ...
  private static buildContinuity(
    scene: DirectorScenePlan,
    prev: DirectorScenePlan | null,
    next: DirectorScenePlan | null,
    plan: DirectorPlan,
  ): ContinuityNotes {
    return {
  ...
  private static buildPrompts(scene: DirectorScenePlan, plan: DirectorPlan): PromptPackage {
  ...
  private static buildFrameDescription(scene: DirectorScenePlan, plan: DirectorPlan): string {
  ...
  private static buildVisualGoal(scene: DirectorScenePlan): string {
  ...
  private static buildGlobalStyle(plan: DirectorPlan): VisualStyle {
    return {
      artStyle: plan.globalStyle,
      renderingStyle: 'cinematic',
      qualityTarget: '1080p',
  ...
  private static buildGlobalContinuity(plan: DirectorPlan): ContinuityNotes {
    return {
      character: plan.characterDescription,
