...
import type {
  ThumbnailPackage, GeneratedThumbnail, ThumbnailSpec,
  ThumbnailAnalysisScore, ThumbnailPlatform,
...
const PLATFORM_SPECS: Record<ThumbnailPlatform, { width: number; height: number; aspectRatio: string }> = {
...
export class ThumbnailPlanner {
  ...
  static async plan(
    ...
    if (videoPath && existsSync(videoPath)) {
      try {
        ...
        log.warn('Frame extraction failed', { error: (err as Error).message?.slice(0, 100) });
      }
    }
    ...
    for (let i = 0; i < thumbPrompts.length; i++) {
      try {
        ...
        // Clean raw
        try { const { unlinkSync } = await import('fs'); unlinkSync(rawPath); } catch { /* ignore */ }
        ...
        log.warn('AI thumbnail generation failed', { index: i, error: (err as Error).message?.slice(0, 80) });
      }
    }
  ...
  private static generatePrompts(topic: string, category: string, colors: string[]): string[] {
  ...
  private static predictCtr(thumb: GeneratedThumbnail): number {
    let score = 50;

    // Text overlay present
    if (thumb.textOverlay && thumb.textOverlay.length > 0) score += 15;
    if (thumb.textOverlay && thumb.textOverlay.length <= 30) score += 5; // Short text = better

    // AI generated tends to be more eye-catching
    if (thumb.generationMethod === 'ai_generated') score += 10;
    if (thumb.generationMethod === 'frame_extract_enhanced') score += 5;

    // File size indicates detail (larger = more detailed)
    if (thumb.sizeBytes > 100000) score += 5;
    if (thumb.sizeBytes > 200000) score += 5;

    // Correct dimensions
    if (thumb.width === 1280 && thumb.height === 720) score += 5;
  ...
  private static analyzeAll(thumbnails: GeneratedThumbnail[], title: string): ThumbnailAnalysisScore {
    ...
    if (!hasAi) improvements.push('Generate AI-specific thumbnail image');
    if (title.length > 30) improvements.push('Shorten thumbnail text to under 30 characters');
  ...
  private static generateShortTitle(topic: string): string {
    // Take first meaningful part, max 30 chars
    const base = topic.split('–')[0]?.trim().split('-')[0]?.trim() ?? topic;
    if (base.length <= 30) return base.toUpperCase();
