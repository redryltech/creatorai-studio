...
export class QualityCheckerAgent implements IAutomationAgent<QualityInput, QualityReport> {
  readonly agentId = 'automation.quality_check';
  readonly agentName = 'Quality Checker';
  readonly stage = 'quality';

  validate(input: QualityInput): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    if (!input.timeline) errors.push('Timeline required');
    if (!input.renderResult) errors.push('Render result required');
  ...
  async execute(input: QualityInput, onProgress: ProgressCallback, _cancellation: CancellationToken): Promise<QualityReport> {
    ...
    if (errorCount > 0) suggestions.push('Fix all error-level issues before publishing');
    if (!timeline.metadata.hasMusic) suggestions.push('Adding background music improves engagement by ~20%');
    if (durationSec > 90) suggestions.push('Videos under 60s perform best on Shorts/Reels/TikTok');
    if (captions.segments.length === 0) suggestions.push('Adding captions increases watch time by ~15%');
