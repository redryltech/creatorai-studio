...
const DEFAULT_PALETTE: EmotionPalette = EMOTION_PALETTES.neutral!;
...
export class MockVideoProvider implements IVideoProvider {
  ...
  readonly priority = 99; // Lowest — real providers override
  ...
  validate(request: VideoGenerationRequest): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    if (!request.sceneId) errors.push('sceneId is required');
    if (!request.durationSec || request.durationSec <= 0) errors.push('durationSec must be > 0');
    if (request.durationSec > 60) errors.push('durationSec must be <= 60');
    if (request.width && request.width < 100) errors.push('width must be >= 100');
    if (request.height && request.height < 100) errors.push('height must be >= 100');
  ...
  async getStatus(): Promise<VideoProviderStatus> {
  ...
  private buildFilterComplex(
    ...
    filters.push(
      `[0:v]zoompan=z='${zoomExpr}':x='${xExpr}':y='${yExpr}'` +
  ...
  private validateOutputFile(
    ...
    try {
      ...
      return { valid: false, error: `FFprobe failed: ${(err as Error).message?.slice(0, 100)}` };
    }
  }
