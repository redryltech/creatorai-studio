...
const RESOLUTIONS: Record<RenderQuality, Record<RenderOrientation, { width: number; height: number }>> = {
...
export class RenderEngineAgent implements IAutomationAgent<RenderInput, RenderResult> {
  readonly agentId = 'automation.render';
  readonly agentName = 'Render Engine';
  readonly stage = 'rendering';

  validate(input: RenderInput): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    if (!input.timeline?.tracks?.length) errors.push('Timeline with tracks required');
    if (input.timeline?.totalDurationMs <= 0) errors.push('Timeline duration must be positive');
    return { valid: errors.length === 0, errors };
  }

  estimateCost(input: RenderInput): { costUsd: number; breakdown: string[] } {
  ...
  async execute(input: RenderInput, onProgress: ProgressCallback, cancellation: CancellationToken): Promise<RenderResult> {
    ...
    try {
      ...
      const projectId = (input.request as Record<string, string>).projectId ?? 'unknown';
      const userId = (input.request as Record<string, string>).userId ?? 'unknown';
