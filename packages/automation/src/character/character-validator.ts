...
export class CharacterValidator {
  static validate(database: CharacterDatabase): CharacterValidationResult {
    ...
    if (!database.id) { errors.push('No database ID'); score -= 10; }
    if (database.entities.length === 0) { warnings.push('No entities detected'); score -= 5; }
    ...
    for (const entity of database.entities) {
      // Duplicate IDs
      if (seenIds.has(entity.id)) { errors.push(`Duplicate entity ID: ${entity.id}`); score -= 10; }
      seenIds.add(entity.id);

      // Duplicate UUIDs
      if (seenUuids.has(entity.uuid)) { errors.push(`Duplicate UUID: ${entity.uuid}`); score -= 10; }
      seenUuids.add(entity.uuid);

      // Required fields
      if (!entity.displayName) { errors.push(`${entity.id}: no display name`); score -= 5; }
      if (!entity.category) { errors.push(`${entity.id}: no category`); score -= 5; }
      if (!entity.identityBlock) { errors.push(`${entity.id}: no identity block`); score -= 10; }
      if (entity.scenePresence.length === 0) { warnings.push(`${entity.id}: appears in no scenes`); score -= 3; }

      // Seed validation
      if (entity.globalSeed === 0) { warnings.push(`${entity.id}: globalSeed is 0`); score -= 2; }
