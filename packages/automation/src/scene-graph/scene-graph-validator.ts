...
import type { SceneGraphPackage, SceneGraph, SceneNode } from './scene-graph.types';
...
export class SceneGraphValidator {
  static validate(pkg: SceneGraphPackage): SceneGraphValidationResult {
    ...
    if (pkg.scenes.length === 0) { errors.push('No scenes'); score -= 50; }
    ...
    for (const graph of pkg.scenes) {
      ...
      // Root exists
      if (!nodeMap.has(graph.rootNodeId)) { errors.push(`${gPrefix}: root node missing`); score -= 10; }

      // Duplicate node IDs
      const ids = new Set<string>();
      for (const [id] of nodeMap) {
        if (ids.has(id)) { errors.push(`${gPrefix}: duplicate node ${id}`); score -= 5; }
        ids.add(id);
      }
      ...
      for (const [, node] of nodeMap) {
        ...
        while (current) {
          if (visited.has(current)) { errors.push(`${gPrefix}: cycle detected at ${node.id}`); score -= 10; break; }
          visited.add(current);
      ...
      for (const rel of graph.relationships) {
        if (!nodeMap.has(rel.sourceNodeId)) { errors.push(`${gPrefix}: relationship ${rel.id} has invalid source`); score -= 3; }
        if (!nodeMap.has(rel.targetNodeId)) { errors.push(`${gPrefix}: relationship ${rel.id} has invalid target`); score -= 3; }
      }

      // Camera validation
      if (!graph.cameraNode) { errors.push(`${gPrefix}: no camera node`); score -= 10; }
      else {
      ...
      // Light validation
      if (graph.lightNodes.length === 0) { warnings.push(`${gPrefix}: no lights`); score -= 3; }
      ...
      // Metrics sanity
      if (graph.metrics.objectCount === 0) { warnings.push(`${gPrefix}: zero objects`); score -= 2; }
    }
