...
import type {
  SceneGraphPackage, SceneGraph, SceneNode, CameraNode, LightNode,
  SceneRelationship, SceneMetrics, Vec3, Rotation, BoundingBox,
  SceneNodeType, LightType, AnimationState, RelationshipType,
...
const rot = (p: number, y: number, r: number): Rotation => ({ pitch: p, yaw: y, roll: r });
const bbox = (min: Vec3, max: Vec3): BoundingBox => ({ min, max });

// Camera position mapping
const CAMERA_POS: Record<string, Vec3> = {
...
const LENS_FOV: Record<string, number> = {
  '24mm': 84, '35mm': 63, '50mm': 47, '85mm': 28, '135mm': 18,
  ultra_wide: 100, telephoto: 15, macro: 40,
};

// Env → node templates
const ENV_NODES: Record<string, Array<{ type: SceneNodeType; name: string; pos: Vec3 }>> = {
...
const LIGHTING_MAP: Record<string, Array<{ type: LightType; name: string; intensity: number; temp: number; pos: Vec3 }>> = {
...
const MOTION_TO_ANIM: Record<string, AnimationState> = {
...
export class SceneGraphPlanner {
  ...
  static plan(
    ...
    const scenes: SceneGraph[] = storyboard.frames.map((frame, idx) => {
    ...
    const globalAnchors: Record<string, Vec3> = {};
    const continuityAnchors: Record<string, { position: Vec3; rotation: Rotation }> = {};
  ...
  private static buildSceneGraph(
    ...
    // ── Environment nodes ──
    const env = (dirScene?.environment ?? frame.style.mood ?? 'studio') as string;
    const envTemplates = ENV_NODES[env] ?? ENV_NODES.studio!;
    const envNodeIds: string[] = [];
    ...
    for (const entity of charDb.entities) {
      ...
      const type: SceneNodeType = entity.category === 'vehicle' ? 'vehicle' :
    ...
    // ── Light nodes ──
    const lightingKey = (dirScene?.lighting ?? 'natural') as string;
    const lightTemplates = LIGHTING_MAP[lightingKey] ?? LIGHTING_MAP.natural!;
    const lightNodes: LightNode[] = lightTemplates.map((lt) => {
      const nid = makeId();
  ...
  private static makeNode(id: string, type: SceneNodeType, name: string, parentId: string | null, pos: Vec3): SceneNode {
    return {
  ...
  private static tempToHex(kelvin: number): string {
    if (kelvin <= 3500) return '#FFB347';
    if (kelvin <= 5000) return '#FFF4E0';
    if (kelvin <= 6500) return '#FFFFFF';
    if (kelvin <= 8000) return '#CCE5FF';
    return '#99CCFF';
  }
}
