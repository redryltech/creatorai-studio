...
import type { CameraNode, Vec3, Rotation, SceneGraph } from './scene-graph.types';
...
export class CameraGraphBuilder {
  ...
  static buildMotionPath(camera: CameraNode, movement: string, duration: number): CameraNode {
    ...
    const keyframes: CameraNode['keyframes'] = [];
    const path: Vec3[] = [];
  ...
  private static interpolatePosition(start: Vec3, movement: string, t: number, dur: number): Vec3 {
  ...
  private static interpolateFov(baseFov: number, movement: string, t: number): number {
    if (movement.includes('zoom_in')) return baseFov - t * 20;
    if (movement.includes('zoom_out')) return baseFov + t * 20;
    return baseFov;
  }
}
