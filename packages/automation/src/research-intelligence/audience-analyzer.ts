...
import type { AudienceAnalysis, AudienceSegment, ContentCategory, Platform } from './research.types';

const log = Logger.for('AudienceAnalyzer');

/** Category → primary audience profile. */
const AUDIENCE_PROFILES: Record<string, { name: string; age: string; gender: AudienceSegment['gender']; interests: string[]; behavior: string; platforms: Platform[]; hours: string; prefs: string[]; size: AudienceSegment['size'] }> = {
...
const DEFAULT_PROFILE = { name: 'General Audience', age: '18-35', gender: 'all' as const, interests: ['entertainment', 'learning'], behavior: 'Casual viewer', platforms: ['youtube_shorts', 'instagram_reels'] as Platform[], hours: '7-10 PM IST', prefs: ['Short engaging content'], size: 'large' as const };

export class AudienceAnalyzer {
  /**
   * Analyze the target audience for a topic.
   */
  static analyze(topic: string, category: ContentCategory): AudienceAnalysis {
    const profile = AUDIENCE_PROFILES[category] ?? DEFAULT_PROFILE;

    const primaryAudience: AudienceSegment = {
      name: profile.name,
    ...
    const secondaryAudiences: AudienceSegment[] = [
      {
