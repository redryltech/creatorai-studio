...
import type { KeywordPackage, KeywordData, ContentCategory } from './research.types';
...
export class KeywordEngine {
  ...
  static generate(topic: string, category: ContentCategory): KeywordPackage {
    ...
    const primary: KeywordData[] = topicWords
      .filter((w) => w.length > 3)
      .slice(0, 5)
      .map((w) => ({
        keyword: w,
    ...
    const secondary: KeywordData[] = seeds
    ...
    const longTail: KeywordData[] = modifiers.slice(0, 8).map((mod, i) => {
      const kw = `${topic.toLowerCase()} ${mod}`;
      return {
        keyword: kw,
        type: 'long_tail' as const,
    ...
    const semantic: KeywordData[] = semanticPool
