...
import type { ContentGapAnalysis, ContentGap, ContentCategory } from './research.types';

const log = Logger.for('ContentGapAnalyzer');

const GAP_TEMPLATES: Record<string, Array<{ gap: string; opportunity: string; angle: string }>> = {
  automotive: [
...
export class ContentGapAnalyzer {
  static analyze(topic: string, category: ContentCategory): ContentGapAnalysis {
    const templates = GAP_TEMPLATES[category] ?? DEFAULT_GAPS;

    const gaps: ContentGap[] = templates.map((t, i) => ({
      gap: t.gap,
      opportunity: t.opportunity,
      competition: (i === 0 ? 'low' : i < 3 ? 'low' : 'medium') as ContentGap['competition'],
      estimatedDemand: Math.max(50, 90 - i * 10),
      suggestedAngle: t.angle,
      priority: (i === 0 ? 'critical' : i < 2 ? 'high' : 'medium') as ContentGap['priority'],
    }));
