...
export interface AnalyticsHistory {
  ...
  trend: { direction: 'growing' | 'stable' | 'declining'; velocity: number };
}
...
export interface LearningMemory {
  ...
  sampleCount: number;
  confidence: number;
  examples: Array<{ contentId: string; value: string; performanceScore: number }>;
  lastUpdated: Date;
}
...
export interface TrendReport {
  ...
  userId: string;
  generatedAt: Date;
  emergingTrends: Array<{ topic: string; platform: string; velocity: number; relevance: number; source: string }>;
  breakingTopics: Array<{ topic: string; urgency: 'high' | 'medium' | 'low'; window: string }>;
  viralOpportunities: Array<{ topic: string; estimatedReach: string; competition: string; suggestedAngle: string }>;
...
export interface AudienceInsight {
  ...
  growthRate: number;
  topInterests: string[];
  peakActivityHours: Array<{ hour: number; day: string; engagement: number }>;
  contentPreferences: Array<{ type: string; engagementRate: number }>;
  demographics: { primaryAge: string; topCountries: string[]; genderSplit: Record<string, number> };
  generatedAt: Date;
}
...
export interface InsightReport {
  ...
  platformBreakdown: Record<AnalyticsPlatform, { views: number; engagement: number; growth: number }>;
  topContent: Array<{ id: string; title: string; views: number; engagement: number; score: number }>;
